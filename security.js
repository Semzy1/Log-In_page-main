...
(function() {
  ...
  const config = {
    // Content Protection
    // Avoid hostile client-side protections by default; prefer server-side controls
  ...
  // Note: Client-side "encryption" is insecure and easily bypassed.
  // Perform all sensitive encryption and tokenization on the server side
  // (use proven libraries such as libsodium, node's crypto, or managed KMS services).
  function encryptData() {
    throw new Error('encryptData removed: do not perform client-side encryption. Use server-side crypto.');
  }

  function decryptData() {
    throw new Error('decryptData removed: do not perform client-side decryption. Use server-side crypto.');
  }
